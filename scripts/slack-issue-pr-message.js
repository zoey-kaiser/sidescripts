...
(function() {
    ...
    const GITHUB_PARENT_CONTAINER = '.rgh-conversation-activity-filter-wrapper'
    const GITHUB_PARENT_STICKY_CONTAINER = '.sticky-content .meta'
    ...
    style.textContent = `
        ...
        .SlackCopyButton:active path {
            fill: green !important;
        }

        .SlackCopyButton.clicked path {
    ...
    const buttonSVGString = `
        <svg width="16px" height="16px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="octicon octicon-copy SlackCopyButton">
    ...
    const observer = new MutationObserver((mutationsList, observer) => {
        for(let mutation of mutationsList) {
            if (mutation.type === 'childList') {
                ensureButtonOnContainer(document.querySelector(GITHUB_PARENT_CONTAINER));
                ensureButtonOnContainer(document.querySelector(GITHUB_PARENT_STICKY_CONTAINER));
                break;
            }
        }
    });
